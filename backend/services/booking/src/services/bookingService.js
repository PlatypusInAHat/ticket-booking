...
const serializeBookingForEvent = (booking) => {
  ...
  if (plainBooking.security) {
    plainBooking.security = {
      riskScore: plainBooking.security.riskScore || 0,
      riskReasons: plainBooking.security.riskReasons || [],
      cloudflareBotScore: plainBooking.security.cloudflareBotScore,
      turnstileVerified: Boolean(plainBooking.security.turnstileVerified)
    };
  }
...
const createBooking = async (bookingData, user, options = {}) => {
  const {
    tickets,
    paymentMethod,
    customerName,
  ...
  const botProtection = options.botProtection || {};
  booking.security = {
    deviceFingerprintHash: botProtection.deviceFingerprintHash || '',
    remoteIpHash: botProtection.remoteIp ? hmacSha256(botProtection.remoteIp) : '',
    riskScore: botProtection.risk?.score || 0,
    riskReasons: botProtection.risk?.reasons || [],
    cloudflareBotScore: botProtection.risk?.cloudflareBotScore ?? undefined,
    turnstileVerified: Boolean(botProtection.turnstile?.success)
  };

  await ensurePurchaseLimitStore();
